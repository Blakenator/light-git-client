...
import {BRANCH_FORMAT, BRANCH_REGEX, COMMIT_FORMAT, WORKTREE_REGEX} from './git.constants';
...
export class GitClient {
  ...
  checkIfGitRepo(): Promise<void> {
    return this.execute(this.getGitPath(), ['rev-parse', '--is-inside-work-tree'], 'Check Is Git Working Tree')
               .then(_.noop)
               .catch(() => {
  ...
  getLocalBranches(): Promise<BranchModel[]> {
    return this.execute(this.getGitPath(), ['branch', BRANCH_FORMAT], 'Get Local Branches')
               .then(output => {
                 let text = output.standardOutput;
        ...
        throw new ErrorModel('getLocalBranches', 'getting the list of locals', err);
      });
  }

  getRemoteBranches(): Promise<BranchModel[]> {
    return this.execute(this.getGitPath(), ['branch', '-r', BRANCH_FORMAT], 'Get Remote Branches')
               .then(output => {
                 let text = output.standardOutput;
                 let match = BRANCH_REGEX.exec(text);
  ...
  getWorktrees(): Promise<WorktreeModel[]> {
    return this.execute(this.getGitPath(), ['worktree', 'list', '--porcelain'], 'Get Worktrees')
               .then(output => {
                 let text = output.standardOutput;
                 ...
                 while (match) {
                   ...
                   worktreeModel.isCurrent = !path.relative(worktreeModel.path, this.workingDir);
               ...
               .catch(err => {
                 throw new ErrorModel('getWorktreeList', 'getting the list of worktrees', err);
               });
  }

  getSubmodules(): Promise<SubmoduleModel[]> {
    return this.execute(this.getGitPath(), ['submodule', 'status', '--recursive'], 'Get Submodules')
               .then(output => {
                 let text = output.standardOutput;
               ...
               .catch(err => {
                 throw new ErrorModel('getSubmoduleList', 'getting the list of submodules', err);
               });
  }

  getStashes(): Promise<StashModel[]> {
    return this.execute(this.getGitPath(), ['stash', 'list'], 'Get Stashes').then(output => {
      let text = output.standardOutput;
