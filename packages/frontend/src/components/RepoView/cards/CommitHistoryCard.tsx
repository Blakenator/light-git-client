...
export const CommitHistoryCard: React.FC<CommitHistoryCardProps> = React.memo(
  ({ repoPath, noMoreCommits }) => {
    ...
    // (e.g. selectedStagedChanges on checkbox click) because updateRepoCache
    // creates a new cache entry object via { ...existing, ...data }.
    ...
    const getTagType = useCallback(
      (tag: string): 'local' | 'remote' | 'tag' | 'head' => {
    ...
    // Memoize callbacks passed to DiffViewer so React.memo is not defeated
    const handleIgnoreWhitespaceClick = useCallback(() => {
      const current = useSettingsStore.getState().settings.diffIgnoreWhitespace;
      handleIgnoreWhitespaceChange(!current);
    }, [handleIgnoreWhitespaceChange]);

    const handleExitCommitOrDiff = useCallback(() => {
      if (onExitDiffView) {
        onExitDiffView();
      } else {
        onToggleView(false);
      }
    }, [onExitDiffView, onToggleView]);
            ...
            <ButtonGroup size="sm">
              {currentBranch && (
                <TooltipTrigger
                  placement="top"
                  overlay={
                    <Tooltip id="tooltip-filter-current-branch">
                ...
                >
                  ...
                  >
                    <Icon
                      name={isCurrentBranchActive ? 'fa-eye-slash' : 'fa-eye'}
                      size="sm"
                    />
                  </Button>
                </TooltipTrigger>
                      ...
                      <ActionsCell onClick={(e) => e.stopPropagation()}>
                        <Dropdown align="end">
                          <ActionsToggle
                            ...
                            size="sm"
                            id={`commit-actions-${commit.hash}`}
                          >
                            <Icon name="fa-ellipsis-vertical" size="sm" />
                          </ActionsToggle>
                          <Dropdown.Menu>
                            <Dropdown.Item
                              onClick={() => handleClickCommit(commit.hash)}
                            >
                            ...
                            <Dropdown.Item
                              onClick={() =>
                                handleCreateBranchFromCommit(commit)
                              }
                            >
                              <Icon
                                name="fa-code-branch"
                            ...
                            <Dropdown.Item
                              onClick={() =>
                                handleResetToCommit(commit, 'soft')
                              }
                            >
                              <Icon
                                name="fa-arrow-left"
                            ...
                            <Dropdown.Item
                              onClick={() =>
                                handleResetToCommit(commit, 'mixed')
                              }
                            >
                              <Icon
                                name="fa-arrow-left"
                            ...
                            <Dropdown.Item
                              ...
                              onClick={() =>
                                handleResetToCommit(commit, 'hard')
                              }
                            >
                              <Icon
