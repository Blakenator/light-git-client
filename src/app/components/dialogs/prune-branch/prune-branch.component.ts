...
export class PruneBranchComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    setTimeout(() => {
      this.modalService.modals['pruneConfirm' + this.repoViewUid].asObservable().subscribe(val => {
        if (val) {
          this.branchesByMerge = this.localBranches.filter(branch => !branch.trackingPath && !branch.isCurrentBranch);
  ...
  getAgeString(branch: BranchModel) {
    let totalHours = (new Date().getTime() - new Date(branch.lastCommitDate).getTime()) / 1000 / 60 / 60;
    let result = [];
