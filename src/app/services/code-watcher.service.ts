...
export class CodeWatcherService {
  ...
  getWatcherAlerts(diff: DiffHeaderModel[]) {
    let watchers = this.settingsService.settings.loadedCodeWatchers.filter(
      (w) => w.enabled,
    );
      ...
      .map((x) => {
        return {
          ...
          hunks: x.hunks
            .map((h) => {
              if (
                this.settingsService.settings.includeUnchangedInWatcherAnalysis
              ) {
                return {
                  hunk: h,
  ...
  getHunkCode(hunk, includeLineNumbers: boolean = false) {
    return hunk.lines
      ...
      .map(
        (l, i) =>
          (includeLineNumbers ? hunk.toStartLine + i + '\t' : '') + l.text,
      )
      .join('\n');
  }
  ...
  public showWatchers(
    request: ShowWatchersRequest = new ShowWatchersRequest(),
  ) {
